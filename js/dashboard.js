...
function switchInspectorTab(tabId) {
  const allowed = ["dashboard", "inspections", "lookup", "reports", "help"];
  ...
  allowed.forEach(id => {
    ...
    if (navBtn) {
      if (id === tabId) {
        navBtn.className = "inspector-nav-btn w-full flex items-center gap-3 px-3.5 py-2.5 rounded-xl bg-amber-500 text-white shadow font-semibold transition text-left";
      } else {
        navBtn.className = "inspector-nav-btn w-full flex items-center gap-3 px-3.5 py-2.5 rounded-xl text-slate-300 hover:bg-slate-800 hover:text-white transition text-left";
      }
    }
  });
